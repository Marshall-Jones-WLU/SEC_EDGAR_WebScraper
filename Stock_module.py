...
Filename: 10kAnalysisOOP_MODULE

Module for analyzing a publicly traded company
"""
...
class Company(object):
    ...
    def get_CIK(self, ticker):
        '''This method uses requests to go to the URL address, and then uses the input ticker to find the
    ...
    def get_filingList(self, self.CIK, docType):
        ...
        filingListSoup = BeautifulSoup(response.content,'xml')
        #print("Response URL:\n"+response.url)
        
        return filingListSoup


    def get_info(self, CIK, docType):
        ...
        soup = Company.get_filingList(self.CIK, docType = '10-k')
    ...
    def get_ACC(self, filingListSoup, counter = 1):
        ...
        # find all the accession number entry tags
        entries = filingListSoup.find_all('entry')
        ...
        for entry in entries:
            ...
            accession_num = entry.find('accession-number').text #SEC used to misspell "number" here
    ...
    def get_summary(self, companyInfoDict, ACC, filingDate, docType, counter = 1):
        ...
        if xml_summary is None:
            ...
            functions again.'''
            counter += 1
            ACC, filingDate = getFilingInfo(companyInfoDict['CIK'], counter, docType)
            xml_summary = get_filing_summary(companyInfoDict, ACC, counter, filingDate, docType)
            if type(xml_summary) is tuple:
                '''I don't know why this creates a tuple'''
                xml_summary = xml_summary[0]
        return xml_summary, counter

    def parse_summary(self, xml_summary, CIK, ACC, filingDate, counter = 1):
    ...
    def scrape_reports(self, statements_url, reportOrder):
    ...
    def footnote_store(self):
        pass


    def make_balSheet_df(self, statements_data, reportOrder):
...
class Analysis(object):
    ...
    def analysis(self, balSheet_df, income_df, cashFlow_df, companyInfoDict, dirPath, filingDate, ticker):
        ...
        cogs2 = None
        cogs3 = None
        cogsKeys = ["Cost of sales","Cost of revenue","Cost of goods sold","COGS","Cost of products sold",
